...
public final class Island extends DatabaseObject {
  ...
  private int getExperienceRequired(int level) {
    ...
    int iter = 0;
    if (!IridiumSkyblock.getInstance().getConfiguration().islandLevelEquation.contains("$1")) {
      return 0;
    }
    while (lower < upper) {
      ...
      if (islandLevelEquation.getVariableCount() == 1) {
        experience = islandLevelEquation.evaluate(mid);
      } else if (islandLevelEquation.getVariableCount() == 0) {
        experience = 0;// Level disabled
      } else if (islandLevelEquation.getVariableCount() == 2) {
        experience = (int) islandLevelEquation.evaluate(mid, getValue());
      } else {
        Throwable t = new Throwable("Island equation cannot contain more than 2 variable");
        t.printStackTrace();
        return 0;
      }
