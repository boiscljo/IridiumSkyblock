...
public class BlockValueGUI extends GUI {
  ...
  public void addContent(Inventory inventory) {
    ...
    inventory.setItem(inventory.getSize() - 3, ItemStackUtils.makeItem(
        IridiumSkyblock.getInstance().getInventories().nextPage, new PlaceholderBuilder().papi(getPlayer()).build()));
    inventory.setItem(inventory.getSize() - 7,
        ItemStackUtils.makeItem(IridiumSkyblock.getInstance().getInventories().previousPage,
    ...
    if (guiType == BlockValueType.BLOCK) {
      IridiumSkyblock.getInstance().getBlockValues().blockValues.entrySet().stream()
          .filter(valuableBlockEntry -> (page == 1 ? valuableBlockEntry.getValue().page <= 1
              : valuableBlockEntry.getValue().page == page))
          .forEachOrdered(valuableBlock -> {
            ObsidianMaterial material = valuableBlock.getKey();
            ValuableBlock blockInfo = valuableBlock.getValue();
            ItemStack blockItem = ItemStackUtils.makeItem(material, 1, StringUtils.color(blockInfo.name),
                StringUtils.processMultiplePlaceholders(getColoredValueLore(blockInfo.value),
                    new PlaceholderBuilder().papi(getPlayer()).build()));
      ...
      IridiumSkyblock.getInstance().getBlockValues().spawnerValues.entrySet().stream()
          .filter(valuableSpawnerEntry -> (page == 1 ? valuableSpawnerEntry.getValue().page <= 1
              : valuableSpawnerEntry.getValue().page == page))
          .forEachOrdered(valuableSpawner -> {
            ValuableBlock spawnerInfo = valuableSpawner.getValue();
            ItemStack spawnerItem = ItemStackUtils.makeItem(ObsidianMaterial.valueOf("SPAWNER"), 1,
                StringUtils.color(spawnerInfo.name), StringUtils.processMultiplePlaceholders(
                    getColoredValueLore(spawnerInfo.value), new PlaceholderBuilder().papi(getPlayer()).build()));
  ...
  public void onInventoryClick(InventoryClickEvent event) {
    ...
    boolean nextPage = (guiType == BlockValueType.SPAWNER ? IridiumSkyblock.getInstance().getBlockValues().spawnerValues
        : IridiumSkyblock.getInstance().getBlockValues().blockValues).entrySet().stream()
        .anyMatch(valueEntry -> valueEntry.getValue().page == page + 1);
  ...
  public enum BlockValueType {
    ...
    /**
     * Returns the category of valuable blocks with the provided name, null if there
     * is none.
     * Case insensitive.
     *
    ...
    public static BlockValueType getType(String type) {
      return Arrays.stream(BlockValueType.values())
          .filter(blockValueType -> blockValueType.name().equalsIgnoreCase(type)).findFirst().orElse(null);
    }
  }
