...
public class BiomeOverviewGUI extends GUI {
  ...
  public Inventory getInventory() {
    Inventory inventory = Bukkit.createInventory(this, IridiumSkyblock.getInstance().getBiomes().overviewSize,
        StringUtils.color(IridiumSkyblock.getInstance().getBiomes().overviewTitle));
  ...
  public void addContent(Inventory inventory) {
    ...
    for (BiomeCategory category : IridiumSkyblock.getInstance().getBiomesManager().getCategories()) {
      inventory.setItem(category.item.slot,
          ItemStackUtils.makeItem(category.item, new PlaceholderBuilder().papi(getPlayer()).build()));
    }
  ...
  public void onInventoryClick(InventoryClickEvent event) {
    IridiumSkyblock.getInstance().getBiomesManager().getCategoryBySlot(event.getSlot())
        .ifPresent(biomeCategory -> IridiumSkyblock.getInstance().getCommands().biomesCommand
            .execute(event.getWhoClicked(), new String[] { "", biomeCategory.name }));
  }
