...
public class BlockFormListener implements Listener {
    ...
    public void onBlockForm(BlockFormEvent event) {
        ...
        if (IridiumSkyblock.getInstance().getConfiguration().useBedrockGeneratorInstead) return;
    ...
    @EventHandler(ignoreCancelled = true)
    public void bedockGenerator(BlockBreakEvent event) {
        if (!IridiumSkyblockAPI.getInstance().isIslandWorld(event.getBlock().getWorld())) return;
        if (IridiumSkyblock.getInstance().getConfiguration().performance.disableGenerator) return;
        if (!IridiumSkyblock.getInstance().getConfiguration().useBedrockGeneratorInstead) return;
        ...
        if (overworld_gen || nether_gen ||end_gen) {
            Optional<Island> island = IridiumSkyblock.getInstance().getIslandManager().getIslandViaLocation(event.getBlock().getLocation());
            if (island.isPresent()) {
                int upgradeLevel = IridiumSkyblock.getInstance().getIslandManager().getIslandUpgrade(island.get(), "generator").getLevel();
                RandomAccessList<ObsidianMaterial> randomMaterialList = null;
                if(overworld_gen) randomMaterialList = normalOreLevels.get(upgradeLevel);
                if(nether_gen) randomMaterialList=netherOreLevels.get(upgradeLevel);
                if(end_gen) randomMaterialList=endOreLevels.get(upgradeLevel);
