...
public class BlockGrowListener implements Listener {
  ...
  private List<ObsidianMaterial> instantGrowCrops = Arrays.asList(ObsidianMaterial.valueOf("CACTUS"),
      ObsidianMaterial.valueOf("SUGAR_CANE"), ObsidianMaterial.valueOf("BAMBOO"));
  ...
  public void monitorBlockGrow(BlockGrowEvent event) {
    if (!IridiumSkyblockAPI.getInstance().isIslandWorld(event.getBlock().getWorld()))
      return;
    if (IridiumSkyblock.getInstance().getConfiguration().performance.disableGrowth)
      return;
        ...
        .ifPresent(island -> {
          ...
          if (event.getNewState().getBlock().getBlockData() instanceof Ageable) {
            ...
            if (ageable.getAge() == ageable.getMaximumAge()
                || instantGrowCrops.contains(ObsidianMaterial.valueOf(event.getNewState().getType()))) {
              IridiumSkyblock.getInstance().getMissionManager().handleMissionUpdates(island, "GROW", material.name(),
                  1);
            }
          } else {
