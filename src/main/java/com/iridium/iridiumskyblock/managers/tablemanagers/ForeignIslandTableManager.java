...
public class ForeignIslandTableManager<T extends IslandData, S> extends TableManager<T, S> {
  ...
  public ForeignIslandTableManager(ConnectionSource connectionSource, Class<T> clazz, Comparator<T> comparator)
      throws SQLException {
  ...
  public void save(T t) {
    super.save(t);
    if (getEntries().contains(t)) {
      getEntries().remove(getEntry(t).get());
      getEntries().add(t);
    }
    addEntry(t);
  }
  ...
  public List<T> getEntries(@NotNull Island island) {
    int index = Collections.binarySearch(getEntries(), new IslandData(island),
        Comparator.comparing(IslandData::getIslandId));
    if (index < 0)
    ...
    while (true) {
      if (currentIndex >= getEntries().size())
        break;
      IslandData islandData = getEntries().get(currentIndex);
      if (islandData == null) {
        currentIndex++;
