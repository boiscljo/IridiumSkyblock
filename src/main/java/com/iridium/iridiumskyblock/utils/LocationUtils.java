...
public class LocationUtils {
  ...
  private static final List<Material> unsafeBlocks = Stream.of(
      ...
      ObsidianMaterial.valueOf("LAVA")).map(ObsidianMaterial::toMaterial).collect(Collectors.toList());
  ...
  public static boolean isSafe(@NotNull Location location) {
    ...
    return multiVersion.isPassable(block) && multiVersion.isPassable(above) && !multiVersion.isPassable(below)
        && !unsafeBlocks.contains(below.getType()) && !unsafeBlocks.contains(block.getType())
   ...
   */
  public static CompletableFuture<Location> getSafeLocation(@NotNull Location location, Island island) {
    CompletableFuture<Location> answer = new CompletableFuture<>();
    Scheduler.getInstance().runChunkTask(location, 0, () -> {
      World world = location.getWorld();
      if (world == null) {
        answer.complete(location);
        return;
      }
      if (island == null) {
        answer.complete(location);
  ...
  public static int getMinHeight(World world) {
    ...
    return VERSION >= 17 ? world.getMinHeight() : 0; // World#getMinHeight() -> Available only in 1.17 Spigot and 1.16.5
                                                     // PaperMC
  }
