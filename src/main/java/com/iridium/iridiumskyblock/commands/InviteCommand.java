...
public class InviteCommand extends Command {
    ...
    public InviteCommand() {
        super(Arrays.asList("invite", "invites"), "Invite a user to your Island", "%prefix% &7/is invite <player>", "",
                true, Duration.ZERO);
    }
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        if (offlinePlayerUser != null) {
            ...
            if (!IridiumSkyblock.getInstance().getIslandManager().getIslandPermission(island.get(), user,
                ...
                player.sendMessage(StringUtils.color(IridiumSkyblock.getInstance().getMessages().cannotInviteMember
                        .replace("%prefix%", IridiumSkyblock.getInstance().getConfiguration().prefix)));
                return false;
            }
            ...
            if (offlinePlayer instanceof Player) {
                ...
                BaseComponent[] message = TextComponent
                        .fromLegacyText(StringUtils.color(IridiumSkyblock.getInstance().getMessages().youHaveBeenInvited
                                .replace("%inviter%", player.getName())
                                .replace("%prefix%", IridiumSkyblock.getInstance().getConfiguration().prefix)));
                for (BaseComponent baseComponent : message) {
                    baseComponent.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND,
                            "/is " + IridiumSkyblock.getInstance().getCommands().joinCommand.aliases.get(0) + " "
                                    + player.getName()));
                    baseComponent.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                            new ComponentBuilder(
                                    StringUtils.color(IridiumSkyblock.getInstance().getMessages().clickToJoinHover))
                                    .create()));
                }
