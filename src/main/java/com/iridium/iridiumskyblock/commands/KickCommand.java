...
public class KickCommand extends Command {
     ...
     */
    public KickCommand() {
        super(Collections.singletonList("kick"), "Kick a player", "%prefix% &7/is kick <player>", "", true,
                Duration.ZERO);
    }
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        if (targetUser != null) {
            ...
            OfflinePlayer targetPlayer = Bukkit.getOfflinePlayer(targetUser.getUuid());
            if (targetPlayer instanceof Player) {
                ((Player) targetPlayer)
                        .sendMessage(StringUtils.color(IridiumSkyblock.getInstance().getMessages().youHaveBeenKicked
            ...
            for (IslandMember member : island.get().getMembers()) {
                ...
                if (islandMember != null) {
                    if (!islandMember.equals(player)) {
                        islandMember
                                .sendMessage(StringUtils.color(IridiumSkyblock.getInstance().getMessages().kickedPlayer
                                        .replace("%kicker%", player.getName()).replace("%player%", targetUser.getName())
                                        .replace("%prefix%", IridiumSkyblock.getInstance().getConfiguration().prefix)));
                    } else {
                        islandMember.sendMessage(
                                StringUtils.color(IridiumSkyblock.getInstance().getMessages().youKickedPlayer
