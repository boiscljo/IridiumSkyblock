...
public class CreateCommand extends Command {
  ...
  public CreateCommand() {
    super(Collections.singletonList("create"), "Create an Island", "%prefix% &7/is create <name> <schematic>", "", true,
    ...
    switch (args.length) {
      ...
      case 3:
        Optional<Schematics.SchematicConfig> schematicConfig = IridiumSkyblock.getInstance().getSchematics().schematics
            .entrySet().stream().filter(entry -> entry.getKey().equalsIgnoreCase(args[2])).map(Map.Entry::getValue)
            .findFirst();
        if (schematicConfig.isPresent()) {
          return createIsland(player, args[1], schematicConfig.get());
        } else {
          player.sendMessage(StringUtils.color(IridiumSkyblock.getInstance().getMessages().islandSchematicNotFound
  ...
  private void createIsland(Player player, @Nullable String name) {
    ...
    if (IridiumSkyblock.getInstance().getSchematics().schematics.size() == 1) {
      SchematicConfig schematicConfig = IridiumSkyblock.getInstance().getSchematics().schematics.values().stream()
          .findFirst().get();
  ...
  /**
   * Creates an island for a specific Player and then teleports them to the island
   * home.
   *
  ...
  private boolean createIsland(Player player, String name, Schematics.SchematicConfig schematicConfig) {
    ...
    if (name != null && IridiumSkyblock.getInstance().getIslandManager().getIslandByName(name).isPresent()) {
      player.sendMessage(StringUtils.color(IridiumSkyblock.getInstance().getMessages().islandWithNameAlreadyExists
          .replace("%prefix%", IridiumSkyblock.getInstance().getConfiguration().prefix)));
      return false;
    }
    ...
    player.sendMessage(StringUtils.color(IridiumSkyblock.getInstance().getMessages().creatingIsland.replace("%prefix%",
