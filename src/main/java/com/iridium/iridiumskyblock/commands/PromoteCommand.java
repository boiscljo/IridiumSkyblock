...
public class PromoteCommand extends Command {
    ...
    public PromoteCommand() {
        super(Collections.singletonList("promote"), "Promote a user", "%prefix% &7/is promote <player>", "", true,
                Duration.ZERO);
    }
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        if (targetUser != null) {
                ...
                if (islandMember != null) {
                    if (islandMember.equals(player)) {
                        islandMember.sendMessage(
                                StringUtils.color(IridiumSkyblock.getInstance().getMessages().promotedPlayer
                                        .replace("%player%", targetUser.getName())
                                        .replace("%rank%", nextRank.getDisplayName())
                                        .replace("%prefix%", IridiumSkyblock.getInstance().getConfiguration().prefix)));
                    } else {
                        islandMember.sendMessage(
                                StringUtils.color(IridiumSkyblock.getInstance().getMessages().userPromotedPlayer
            ...
            IslandLog islandLog = new IslandLog(island.get(), LogAction.USER_PROMOTED, user, targetUser, 0,
