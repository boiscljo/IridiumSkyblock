...
public class DeleteCommand extends Command {
    ...
    public DeleteCommand() {
        super(Collections.singletonList("delete"), "Delete your Island", "%prefix% &7/is delete <name>", "", false,
    ...
    public boolean execute(CommandSender sender, String[] args) {
        if (args.length == 2 && sender.hasPermission("iridiumskyblock.delete.other")) {
            ...
            if (!islandOptional.isPresent()) {
                ...
                sender.sendMessage(StringUtils.color(IridiumSkyblock.getInstance().getMessages().otherIslandDeleted
                        .replace("%prefix%", IridiumSkyblock.getInstance().getConfiguration().prefix)));
            }
        ...
        if (!(sender instanceof Player)) {
            sender.sendMessage(StringUtils.color(IridiumSkyblock.getInstance().getMessages().notAPlayer)
                    .replace("%prefix%", IridiumSkyblock.getInstance().getConfiguration().prefix));
            return false;
        }
