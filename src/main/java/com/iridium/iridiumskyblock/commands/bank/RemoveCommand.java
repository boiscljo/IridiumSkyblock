...
public class RemoveCommand extends Command {
    ...
    public RemoveCommand() {
        super(Collections.singletonList("remove"), "Remove value from a players bank",
                "%prefix% &7/is bank remove <player> <type> <amount>", "iridiumskyblock.bank.remove", false,
                Duration.ZERO);
    }
    ...
    public boolean execute(CommandSender sender, String[] args) {
        if (args.length == 5) {
            ...
            if (user != null) {
                ...
                if (island.isPresent()) {
                    Optional<BankItem> bankItem = IridiumSkyblock.getInstance().getBankItemList().stream()
                            .filter(item -> item.getName().equalsIgnoreCase(args[3])).findFirst();
                    if (bankItem.isPresent()) {
                        try {
                            IslandBank islandBank = IridiumSkyblock.getInstance().getIslandManager()
                                    .getIslandBank(island.get(), bankItem.get());
                            islandBank.setNumber(Math.max(islandBank.getNumber() - Double.parseDouble(args[4]), 0));
                            sender.sendMessage(StringUtils.color(IridiumSkyblock.getInstance().getMessages().removedBank
                                    .replace("%player%", user.getName()).replace("%amount%", args[4])
                        ...
                        sender.sendMessage(StringUtils.color(IridiumSkyblock.getInstance().getMessages().noSuchBankItem
                                .replace("%prefix%", IridiumSkyblock.getInstance().getConfiguration().prefix)));
                    }
                } else {
